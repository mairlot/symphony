			...
			CmlInterpreterException
	{
		...
		switch (mode)
		{
			case CoSimCoordinator:
			{
				...
				factory = new CoSimCoordinatorInterpreterFactory();
				factory.setDefaultCmlBehaviourFactory(new CoSimCmlBehaviorFactory(server));

				break;
			}
			case CoSimClient:
			{
				CoSimulationClient client = new CoSimulationClient(coSimPort);
				client.connect();
				client.start();
